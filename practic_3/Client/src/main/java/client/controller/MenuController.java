...
public class MenuController {
    ...
    @FXML
    void initialize() {
        http.get("/games/leaderboard",
                response -> {
                    JsonObject jsonObject = new JsonParser().parse(response).getAsJsonObject();
                    top1.setText(jsonObject.get("top1").getAsString());
                    top2.setText(jsonObject.get("top2").getAsString());
                    top3.setText(jsonObject.get("top3").getAsString());
                },
                error -> showAlert("Failed to load leaderboard")
                );
    }
